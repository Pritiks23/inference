  ...
  container.innerHTML = `
    <form id="userInputForm" class="user-input-form">
      ...
      <label>Latency Target (sec)
        <input type="number" id="latency" value="10" min="1" step="1" required>
      </label>
...
function showResults() {
  ...
  const costPref = document.getElementById('costPref').value;
  ...
  if (costPref !== 'any') {
    filtered = clusters.filter(c => {
      if (costPref === 'low') return c.rate <= 3;
      if (costPref === 'medium') return c.rate > 3 && c.rate <= 7;
      if (costPref === 'high') return c.rate > 7;
      return true;
    });
  }
  filtered.forEach(c => {
    const optThroughput = c.base * (1 + batchingGains[c.name] * similarity) * (1 + 0.01 * (batchSize - 1));
    const timeNeeded = tokens / optThroughput;
    const optCost = timeNeeded * c.rate / 3600;
    avgCost += optCost;
  });
  avgCost = avgCost / filtered.length;
  ...
  document.getElementById('resultsTable').innerHTML = `
    ...
    <div class="results-summary">
      <b>Summary:</b> Showing ${filtered.length} clusters. Optimization based on ${tokens.toLocaleString()} tokens, similarity ratio ${similarity}, batch size ${batchSize}, latency target ${latency}s, cost preference: ${costPref}.
    </div>
  `;
}
