...
function showResults() {
  ...
  clusters.forEach(c => {
    ...
    const perToken = +(optCost / tokens).toExponential(2); // FIX: cost_per_token = total_cost / total_tokens
    clusterResults.push({
      ...c,
      optThroughput,
      timeNeeded,
  ...
  let rows = clusterResults.map((c, i) => {
    return `<tr><td>${i+1}</td><td>${c.name}</td><td>${c.gpu}</td><td>${c.base}</td><td>${c.optThroughput}</td><td>${c.timeNeeded}</td><td>${c.perToken}</td><td>${c.optCost}</td><td>${(avgCost - c.optCost).toFixed(2)}</td></tr>`;
  }).join('');
