...
function showResults() {
  ...
  // Dynamic explanation
  const why = [
    `Meets latency constraint (user target: ${Math.round(minTime)}s)`,
    `Best cost per token under workload`,
    `High KV-cache reuse efficiency (${document.getElementById('similarity').value} similarity)`
  ];
  ...
  const rankRows = [
    ...
    `<tr><td>🥇</td><td>${best.name.replace(/-.*/, '')}</td><td>Cheapest</td></tr>`,
    `<tr><td>⚡</td><td>${fastest.name.replace(/-.*/, '')}</td><td>Fastest</td></tr>`,
    `<tr><td>⚖️</td><td>${bestBalance.name.replace(/-.*/, '')}</td><td>Best balance</td></tr>`
  ].join('');
