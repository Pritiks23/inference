...
function showResults() {
  ...
  // Production-ready: select best cluster by lowest optimized cost for user tokens
  let best = clusters[0];
  let minCost = best.optCost * (tokens / 1000000);
  for (const c of clusters) {
    const cost = c.optCost * (tokens / 1000000);
    if (cost < minCost) {
      best = c;
      minCost = cost;
    }
